...
class Mk1MultitaskBox(VecTask):
    ...
    def _create_envs(self, spacing: float, num_per_row: int) -> None:
        ...
        for i in range(self.num_envs):
            ...
            self.gym.create_actor(env_ptr, box_asset, box_pose, "box", i, 0, 0)
    ...
    def post_physics_step(self):
        ...
        self.actor_obs["linear"][:], self.critic_obs["linear"][:] = compute_linear_robot_observations(
            robot_root_states = self.root_states[robot_ids], 
            robot_sensor_states=self.vec_force_sensor_tensor,
    ...
    def apply_domain_randomization(self, env_ids: torch.Tensor):
        ...
        # update the initial root state linear velocities with the randomized values given by the task std
        self.initial_root_states[robot_ids , 7] = self.intitial_velocities[0]()  # vel in -y axis
        self.initial_root_states[robot_ids , 8] = self.intitial_velocities[1]() # vel in -x axis
        self.initial_root_states[robot_ids , 9] = self.intitial_velocities[2]() # vel in -z axis
    ...
    def allocate_buffers(self):
        ...
        # all the env ids, (notice not the robot ids), basically just an count up to the total amount of envs 
        self.all_env_ids = torch.arange(start=0, end= self.num_envs).to(self.device).to(dtype=torch.int64)
    ...
    def _compute_robot_rewards(self) -> Tuple[torch.Tensor, torch.Tensor,]:
        ...
        has_fallen = torch.where(self.root_states[robot_ids, 2] < terminations_height, torch.ones_like(reward,  dtype=torch.int8) , torch.zeros_like(reward, dtype=torch.int8))
    ...
    def _get_standard_config(self) -> Dict:
        ...
        base_config_path = os.path.join(dirname, 'config_mk1_multi_box.yaml')
...
def compute_linear_robot_observations(
                                ...
                                robot_sensor_states: torch.Tensor, 
    ...
    torso_position = robot_root_states[:, 0:3]
    torso_rotation = robot_root_states[:, 3:7]
    velocity = robot_root_states[:, 7:10]
    ang_velocity = robot_root_states[:, 10:13]
    ...
    linear_actor_obs = torch.cat((robot_sensor_states.view(robot_root_states.shape[0], -1), dof_pos, dof_vel, dof_force, ang_velocity, torso_rotation, actions, torso_position), dim=-1)
