...
class Mk1ControlledTask(Mk1BaseClass):
    ...
    def reset_envs(self, env_ids: torch.Tensor, do_reset_bool_tensor: torch.Tensor) -> None:
        return super().reset_envs(env_ids, do_reset_bool_tensor)
    
        # reset the target position and the target velocity
    

    def get_additional_critic_obs(self) -> Dict[str, torch.Tensor]:
        
        # command [0] is heading velocity
        # command [1] is x pos target 
        # command [2] is y pos target
