...
from testing_env.base_env.base_env import BaseEnv
from testing_env.common.utils import  join_configs
...
class VecTask(BaseEnv, ABC):
    ...
    def allocate_buffers(self):
        ...
        self.objective_rewards = torch.zeros((self.num_envs, ) , device= self.device, dtype=torch.float32)
        ...
        self.cumulative_objective_rewards = torch.zeros(self.num_envs, device= self.device, dtype= torch.float32)
    ...
    def set_viewer(self):
        ...
        if self.headless == False:
            ...
            if sim_params.up_axis == gymapi.UP_AXIS_Z:
                ...
                cam_target = gymapi.Vec3(0.0, 10.0, 1.0)
            else:
    ...
    def step(self, actions: torch.Tensor) -> Tuple[Dict[str, torch.Tensor], torch.Tensor, torch.Tensor, Dict[str, Any], Dict[str, int]]:    
        ...
        self.cumulative_objective_rewards += self.objective_rewards
        self.masked_cumulative_objective_rewards = self.do_reset * self.cumulative_objective_rewards
        # reset the cumulative rewards for the do_reset values
        self.cumulative_objective_rewards = self.cumulative_objective_rewards * (1- self.do_reset)
        ...
        self.extras["objective_episode_reward"] = self.masked_cumulative_objective_rewards.to(self.rl_device)
        ...
        return self.actor_obs, self.rewards, self.do_reset, self.extras, self.reward_constituents
        
    def reset(self) -> Tuple[Dict[str, torch.Tensor]]:
        """Reset the environment 

        Returns:
            Tuple[Dict[str, torch.Tensor]]: actor_obs
        """
