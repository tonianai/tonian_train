...
from testing_env.mk1_base.mk1_base import Mk1BaseClass
from testing_env.base_env.vec_task import VecTask 
from testing_env.common.torch_jit_utils import batch_dot_product
...
class Mk1RunningTask(Mk1BaseClass):
    ...
    def _extract_params_from_config(self) -> None:
        ...
        self.forward_speed_factor = reward_weight_dict["arm_position_cost"]
    ...
    def _compute_robot_rewards(self) -> Tuple[torch.Tensor, torch.Tensor,]:
        ...
        direction_reward_precomp = torch.where(torch.logical_and(upright_value > 0.7, heading_to_velocity_angle < 0.5), vel_norm , torch.zeros_like(reward))

        direction_reward = direction_reward_precomp * self.forward_speed_factor
    ...
    def get_obs(self) -> Tuple[ Dict[str, torch.Tensor], Dict[str, torch.Tensor]]:
        ...
        self.actor_obs["linear"][:] = compute_linear_robot_observations(
